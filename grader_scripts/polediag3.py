...
problemconfig = sketchresponse.config({
    ...
    'plugins': [
        ...
        {'name': 'stamp', 'id': 'e', 'label': 'Poles', 'scale':.2, 'src':'/static/stamp_x.svg', 'iconsSrc':'/static/stamp_x-icon.svg'}  
    ]
})
...
def grader(e):
    ...
    if e.get_number_of_points() != 3:
        return False, '<font color="blue">You do not have the correct number of poles.<br/></font>'

    if not e.has_point_at(x=2,y=0) or not e.has_point_at(x=-3,y=-1) or not e.has_point_at(x=-3,y=1):
