...
The 'polyline' plugin entry enables the a tool to draw either a multi-segment polyline, or a polygon depending on its configuration. In this problem we are using a polyline to intialize beam object and a polygon that the students will use to draw an isolation boundary. It has five or size configuration options to set here:
...
### Checking the number of isolation boundaries drawn
...
This first check verifies that there is only one isolation boundary drawn in the input data.

### Checking the intersections of the isolation boundary with the beams
...
There are two valid isolation boundaries that can be drawn for this problem. In both of them, the vertical beam should not be cut by the boundary and the horizontal beam should be cut at the point labeled with the vertical dashed line.

### Checking the beam points on the isolation boundary
...
The correct isolation boundarys will have the point on the horizontal beam marked by the vertical dashed line on the boundary. It will also have either the top endpoint of the vertical beam, or the right end point of the horizontal beam on its boundary.
